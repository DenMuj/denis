...
#include "real3d-mpi-qf-mu-nono-K3-Q5s-sprng.h"
...
int main(int argc, char **argv) {
   ...
   fprintf(out, "----------------------------------------------------------------------------------\n");
   fprintf(out, "Snap      Norm0          Norm           mu             <r>            npeak \n");
   fprintf(out, "----------------------------------------------------------------------------------\n");
   fflush(out);
   ...
   npeak = cabs(psi[localNx - 1][Ny2][Nz2]);
   npeak *= npeak;
   MPI_Bcast(&npeak, 1, MPI_DOUBLE, nprocs / 2 - 1, MPI_COMM_WORLD);

   fprintf(out, "%-9d %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", 0, norm_psi2, norm_psi2 + h4 * norm_psi3, mutot, *rms / sqrt(norm_psi2), npeak);
   fflush(out);
   ...
   for (snap = 1; snap <= Nsnap; snap ++) {
      ...
      calcnonorm(psi, tmpxi, tmpyi, tmpzi);
      calcrms(rms, psi, psi_t, tmpxi, tmpyi, tmpzi);
      ...
      fprintf(out, "%-9li %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", snap, norm_psi2, norm_psi2 + h4 * norm_psi3, mutot, *rms / sqrt(norm_psi2), npeak);
      fflush(out);
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("K3")) == NULL) {
      fprintf(stderr, "K3 is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   K3 = atof(cfg_tmp);
...
void calcnonorm(double complex ***psi, double **tmpx, double **tmpy, double **tmpz) {
   int threadid;
   long cnti, cntj, cntk;
   double tmp;
   void *sendbuf;
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            psi[cnti][cntj][cntk] *= cexp(- I * tmp - dt * g3 * psi2 * psi2);
         }
      }
   }
