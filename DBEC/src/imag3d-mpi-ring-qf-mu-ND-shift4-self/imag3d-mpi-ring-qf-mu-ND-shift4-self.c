...
#include "imag3d-mpi-ring-qf-mu-ND-shift4-self.h"
...
void initpsi(double ***psi) {
   ...
   if (input != NULL) {
      ...
      #pragma omp parallel for private(cnti, cntj, cntk, tmp, s4)
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            s4 = sin(0.2 + ND * 0.5 * atan2(y[cntj], x[offsetNx + cnti])); s4 *= s4; s4 *= s4;
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = sqrt(x2[offsetNx + cnti] + y2[cntj]) - Rring;
               psi[cnti][cntj][cntk] = (1. + 5. * s4) * exp(- 0.5 * (tmp * tmp / (ueta * ueta) + z2[cntk] / (uz * uz))) / cpsi;
            }
         }
      }
   }
