...
#include "imag3d-mpi-qf-bel-mu-self.h"
...
int main(int argc, char **argv) {
   ...
   h2bel0 = 32. * sqrt(pi) * pow(as  * BOHR_RADIUS / aho, 2.5) * pow(Na, 1.5) / 3.;
   deltabel0 = 16. * pow(as  * BOHR_RADIUS / aho, 1.5) * sqrt(Na) * q5;
...
double q3fun(double xedd) {

   return QF * (creal((-(sqrt(3.)*pow(-1. + xedd,2)*clog(-3.*(-1. + xedd)*xedd)) + 2*(sqrt(xedd)*(5. + xedd)*sqrt(1. + 2.*xedd) +
          sqrt(3.)*pow(-1. + xedd,2)*log(3.*xedd + sqrt(3.)*sqrt(xedd)*sqrt(1. + 2.*xedd))))/ (16.*sqrt(xedd))));
}

double q5fun(double xedd) {

   return QF * (creal((6.*sqrt(1. + 2.*xedd)*(11. + xedd*(4. + 9.*xedd)) + (5.*sqrt(3.)*pow(-1. + xedd,3)*(clog(-3.*(-1. + xedd)*xedd) -
          2.*log(3.*xedd + sqrt(3.)*sqrt(xedd)*sqrt(1. + 2.*xedd))))/sqrt(xedd))/96.));
}
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            psi2lin = g * psi2;
            psidd2lin = gd * psidd2[cnti][cntj][cntk];
            deltabel = deltabel0 * psi[cnti][cntj][cntk];
            h2bel = h2bel0 * (4. * pow(1. + deltabel, 2.5) * q5fun(edd / (1. + deltabel)) + pow(1. + deltabel, 1.5) * q3fun(edd / (1. + deltabel)));
            tmp = dt * (pot[cnti][cntj][cntk] + psi2lin + psidd2lin + h2bel * psi3);
