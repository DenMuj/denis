...
#include "real3d-mpi-ring-qf-mu-exp-sprng.h"
...
int main(int argc, char **argv) {
   ...
   nstreams = nans * nprocs;
   streamnum = nanscurr * nprocs + rank;
   ...
   fprintf(out, "\n**********************************************\n");
   fprintf(out, "Real-time propagation in 3D, OPTION = %d\nMPI nodes = %d, OMP threads = %d, cores = %d\n", opt, nprocs, nthreads, nprocs * nthreads);
   fprintf(out, "**********************************************\n\nInteractions\n");

   if (cfg_read("G0") != NULL) {
      fprintf(out, "Contact: G0 = %.6le, G1 = %.6le\n", g0, g1);
   } else {
      fprintf(out, "Contact: Natoms = %.6le, as0 = %.6le * a0, as1 = %.6le * a0, G0 = %.6le, G1 = %.6le\n", (double) Na, as0, as1, g0, g1);
   }
   ...
   fprintf(out, "\nTime discretization: NITER1 = %li, NITER2 = %li (NSNAP = %li)\n", Niter1, Niter2, Nsnap);
   fprintf(out, "                     DT = %.6le\n",  dt);
   if (cfg_read("TAU") != NULL) fprintf(out, "       Unit of time: tau = %.6le s\n", tau);
   fprintf(out, "\nInitial state: file %s\n\n", input);
   ...
   if (rmsout != NULL) {
      fprintf(filerms, "\n**********************************************\n");
      fprintf(filerms, "Real-time propagation in 3D, OPTION = %d\nMPI nodes = %d, OMP threads = %d, cores = %d\n", opt, nprocs, nthreads, nprocs * nthreads);
      fprintf(filerms, "**********************************************\n\nInteractions\n");

      if (cfg_read("G0") != NULL) {
         fprintf(filerms, "Contact: G0 = %.6le, G1 = %.6le\n", g0, g1);
      } else {
         fprintf(filerms, "Contact: Natoms = %.6le, as0 = %.6le * a0, as1 = %.6le * a0, G0 = %.6le, G1 = %.6le\n", (double) Na, as0, as1, g0, g1);
      }
   ...
   calcnorm(&norm, psi, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj);
   donorm(norm, psi);
   norm_psi2 *= norm * norm;
   ...
   nsteps = (Niter1 + Niter2) / Nsnap;
   for (itercurr = 0, snap = 1; snap <= Nsnap; snap ++) {
   ...
   if (finalden != NULL) {   
       sprintf(filename, "%s.bin", finalden);
       MPI_File_open(MPI_COMM_WORLD, filename, MPI_MODE_WRONLY | MPI_MODE_CREATE, MPI_INFO_NULL, &mpifile);
       outdenxyz(psi, outxyz, mpifile);
       MPI_File_close(&mpifile);
   }
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("NANS")) == NULL) {
      fprintf(stderr, "NANS is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   nans = atol(cfg_tmp);

   if ((cfg_tmp = cfg_read("NANSCURR")) == NULL) {
      fprintf(stderr, "NANSCURR is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   nanscurr = atol(cfg_tmp);
   ...
   if ((cfg_tmp = cfg_read("G0")) == NULL) {
      if ((cfg_tmp = cfg_read("AS0")) == NULL) {
         fprintf(stderr, "AS0 is not defined in the configuration file.\n");
         exit(EXIT_FAILURE);
      }
      as0 = atof(cfg_tmp);

      g0 = 4. * pi * as0 * Na * BOHR_RADIUS / aho;
   } else {
      g0 = atof(cfg_tmp);
   }

   if ((cfg_tmp = cfg_read("G1")) == NULL) {
      if ((cfg_tmp = cfg_read("AS1")) == NULL) {
         fprintf(stderr, "AS1 is not defined in the configuration file.\n");
         exit(EXIT_FAILURE);
      }
      as1 = atof(cfg_tmp);

      g1 = 4. * pi * as1 * Na * BOHR_RADIUS / aho;
   } else {
      g1 = atof(cfg_tmp);
   }

   if ((cfg_tmp = cfg_read("RRING")) == NULL) {
      fprintf(stderr, "RRING is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   Rring = atof(cfg_tmp) / aho;
...
/*
   ...
   vnu = atof(cfg_tmp);
*/
   if ((cfg_tmp = cfg_read("ETA")) == NULL) {
      fprintf(stderr, "ETA is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   veta = atof(cfg_tmp);
   ...
   if ((cfg_tmp = cfg_read("LAMBDA")) == NULL) {
      ...
      exit(EXIT_FAILURE);
   } else vlambda = atof(cfg_tmp);

   if ((cfg_tmp = cfg_read("NITER1")) == NULL) {
      fprintf(stderr, "NITER1 is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   Niter1 = atol(cfg_tmp);

   if ((cfg_tmp = cfg_read("NITER2")) == NULL) {
      fprintf(stderr, "NITER2 is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   Niter2 = atol(cfg_tmp);
   ...
   if ((cfg_tmp = cfg_read("INPUT")) == NULL) {
      fprintf(stderr, "INPUT is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   input = cfg_read("INPUT");

   if ((cfg_tmp = cfg_read("INPUT_TYPE")) == NULL) {
      fprintf(stderr, "INPUT_TYPE is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   ...
   if (Niterout != NULL) {
      ...
      outflags = (enum OutputFlags) atoi(cfg_tmp);  
   } else outflags = (enum OutputFlags) 0;

   if ((Niterout != NULL) || (finalden != NULL) || (finalpsi != NULL)) {   
...
void updateqf(void) {
   ...
   h2 = par * 32. * sqrt(pi) * pow(as  * BOHR_RADIUS / aho, 2.5) * pow(Na, 1.5) * (4 * q5 + q3) / 3.;
...
void initpsi(double complex ***psi) {
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            psi[cnti][cntj][cntk] *= (1. + 0.01 * stream -> sprng());
         }
      }
   }  
...
void initpot() {
   long cnti, cntj, cntk;
   double veta2, vlambda2, tmp;
   //double vgamma2, vnu2, vlambda2, tmp;
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            tmp = sqrt(x2[offsetNx + cnti] + y2[cntj]) - Rring;
            pot[cnti][cntj][cntk] = 0.5 * par * (veta2 * tmp * tmp + vlambda2 * z2[cntk]);
         }
      }
   }
 ...
 *    tmpz - temporary array
 */
void calcnorm(double *norm, double complex ***psi, double **tmpx, double **tmpy, double **tmpz, double **tmpx3, double **tmpy3, double **tmpz3) {
   int threadid, quick;
   long cnti, cntj, cntk;
   double tmp, alpha;
   void *sendbuf;
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpz3[threadid][cntk] = tmp * tmp * tmp;
            }
            tmpy[threadid][cntj] = simpint(dz, tmpz[threadid], Nz);
            tmpy3[threadid][cntj] = simpint(dz, tmpz3[threadid], Nz);
         }
         (*tmpx)[cnti] = simpint(dy, tmpy[threadid], Ny);
         (*tmpx3)[cnti] = simpint(dy, tmpy3[threadid], Ny);
      }
   }

   sendbuf = (rank == 0) ? MPI_IN_PLACE : *tmpx;
   MPI_Gather(sendbuf, localNx, MPI_DOUBLE, *tmpx, localNx, MPI_DOUBLE, 0, MPI_COMM_WORLD);
   sendbuf = (rank == 0) ? MPI_IN_PLACE : *tmpx3;
   MPI_Gather(sendbuf, localNx, MPI_DOUBLE, *tmpx3, localNx, MPI_DOUBLE, 0, MPI_COMM_WORLD);

   if (rank == 0) {
      norm_psi2 = simpint(dx, *tmpx, Nx);
      norm_psi3 = simpint(dx, *tmpx3, Nx);
...
void donorm(double norm, double complex ***psi) {
   long cnti, cntj, cntk;
   ...
   #pragma omp parallel private(threadid, cnti, cntj, cntk)
   {
      ...
      for (cntj = 0; cntj < localNy; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            for (cnti = 0; cnti < Nx; cnti ++) {
               dpsi_t[cnti][cntj][cntk] = (double) (tmpdpsi2[threadid][cnti] * conj(tmpdpsi2[threadid][cnti]));
            }
         }
      }
   }
   ...
   #pragma omp parallel private(threadid, cnti, cntj, cntk)
   {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            for (cntj = 0; cntj < Ny; cntj ++) {
               dpsi[cnti][cntj][cntk] += (double) (tmpdpsi2[threadid][cntj] * conj(tmpdpsi2[threadid][cntj]));
            }
         }
      }
      #pragma omp barrier
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzmui[3][threadid][cntk] = gd * psidd20[cnti][cntj][cntk] * psi2 / (Nx * Ny * Nz);
               tmpzmui[4][threadid][cntk] = h2 * psi3 * psi2;
               tmpzmui[5][threadid][cntk] = gd * (psidd2[cnti][cntj][cntk] - psidd20[cnti][cntj][cntk]) * psi2 / (Nx * Ny * Nz);
            }
...
void calcmu(double *mu, double complex ***psi, double complex ***psi_t, double ***dpsi, double ***dpsi_t, double ***psidd2, fftw_complex *psidd2fft, double **tmpxi, double **tmpyi, double **tmpzi, double **tmpxj, double **tmpyj, double **tmpzj, double complex **tmpdpsi1, double complex **tmpdpsi2) {
   ...
   #pragma omp parallel private(threadid, cnti, cntj, cntk, psi2, psi3, dpsi2, psi2lin, psidd2lin, tmp)
   {
      threadid = omp_get_thread_num();
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               psi3 = psi2 * tmp;
               psi2lin = g * psi2;
               psidd2lin = gd * psidd2[cnti][cntj][cntk] / (Nx * Ny * Nz);
...
void calcnu(double complex ***psi, double ***psidd2, fftw_complex *psidd2fft) {
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            tmp = cabs(psi[cnti][cntj][cntk]) * norm;
            psi2 = tmp * tmp;
            psi3 = psi2 * tmp;
            psi2lin = g * psi2;
            psidd2lin = gd * psidd2[cnti][cntj][cntk] / (Nx * Ny * Nz);
