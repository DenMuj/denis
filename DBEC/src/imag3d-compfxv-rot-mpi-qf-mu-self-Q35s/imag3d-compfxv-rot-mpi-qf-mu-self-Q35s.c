...
#include "imag3d-compfxv-rot-mpi-qf-mu-self-Q35s.h"
...
int main(int argc, char **argv) {
   ...
   fprintf(out, "Parameters of rotation: OMEGA = %.6e\n\n", omega);
   ...
   if (rmsout != NULL) {
      ...
      fprintf(filerms, "Parameters of rotation: OMEGA = %.6e\n\n", omega);
...
void initpsi(double complex ***psi) {
   ...
   if (input != NULL) {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = exp(- 2.0 * (x2[offsetNx + cnti] / (ux * ux) + y2[cntj] / (uy * uy) + z2[cntk] / (uz * uz)));
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            psi[cnti][cntj][cntk] *= (y[cntj] + I * z[cntk]) / sqrt(y2[cntj] + z2[cntk] + 2 * xi * xi);
         }
      }
   }
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            ...
            for (cntk = Nz - 2; cntk > 0; cntk --) {
               ...
               cbeta[threadid][cntk - 1] =  cgammaz[cntk][cntj] * (Azm[cntj] * cbeta[threadid][cntk] - c);
            }
            psi[cnti][cntj][0] = 0.;
