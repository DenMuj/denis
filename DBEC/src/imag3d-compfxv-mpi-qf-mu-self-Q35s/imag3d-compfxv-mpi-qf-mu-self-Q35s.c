...
#include "imag3d-compfxv-mpi-qf-mu-self-Q35s.h"
...
void initpsi(double complex ***psi) {
   ...
   if (input != NULL) {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = exp(- 2.0 * ((x2[offsetNx + cnti] + y2[cntj]) / (urho * urho) + z2[cntk] / (uz * uz)));
               psi[cnti][cntj][cntk] = cpsi * sqrt(x2[offsetNx + cnti] + y2[cntj]) * cexp(I * atan2(y[cntj], x[offsetNx + cnti])) * tmp;
            }
         }
      }
   }
