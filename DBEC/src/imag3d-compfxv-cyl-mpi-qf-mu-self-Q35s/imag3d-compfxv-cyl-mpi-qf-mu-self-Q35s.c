...
int main(int argc, char **argv) {
   ...
   psiy = alloc_complex_matrix(localNx, Ny2);
...
void gencoef(void) {
   ...
   calphay[0] = 1.;
   cgammay[0] = - 1. / (Ay0 + Ay + dAy / dy);
   for(cnti = 1; cnti < Ny2 - 1; cnti ++) {
      calphay[cnti] = (Ay - dAy / (dy * cnti)) * cgammay[cnti - 1];
      cgammay[cnti] = - 1. / (Ay0 + (Ay + dAy / (dy * (cnti + 1))) * calphay[cnti]);
   }
 ...
 *    cbeta - Crank-Nicolson scheme coefficients
 */
void calcluy(double complex ***psi, double complex **cbeta, double complex **psiy) {
   int threadid;
   ...
   #pragma omp parallel for private(cnti, cntj)
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = Ny2; cntj < Ny; cntj ++) {
         psiy[cnti][cntj - Ny2] = psi[cnti][cntj][Nz2];
      }
   }
   ...
   #pragma omp parallel private(threadid, cnti, cntj, c)
   {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         cbeta[threadid][0] = 0.;
         for (cntj = 1; cntj < Ny2 - 1; cntj ++) {
            c = psiy[cnti][cntj] - Ay * (psiy[cnti][cntj + 1] - 2 * psiy[cnti][cntj] + psiy[cnti][cntj - 1]) + dAy / (dy * cntj) * (psiy[cnti][cntj + 1] - psiy[cnti][cntj - 1]);
            cbeta[threadid][cntj] = cgammay[cntj - 1] * ((Ay + dAy / (dy * cntj)) * cbeta[threadid][cntj - 1] - c);
         }
         psiy[cnti][Ny2 - 1] = 0.;
         for (cntj = Ny2 - 1; cntj > 0; cntj --) {
            psiy[cnti][cntj - 1] = calphay[cntj - 1] * psiy[cnti][cntj] + cbeta[threadid][cntj - 1];
         }
      }
   }

   #pragma omp parallel for private(cnti, cntj, cntk, rho, cntl)
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            rho = sqrt(y2[cntj] + z2[cntk]);
            if (rho > maxrho) {
               psi[cnti][cntj][cntk] = 0.;
            }
            else {
               cntl = (long) (rho / dy);
               psi[cnti][cntj][cntk] = psiy[cnti][cntl];
            }
         }
      }
   }
