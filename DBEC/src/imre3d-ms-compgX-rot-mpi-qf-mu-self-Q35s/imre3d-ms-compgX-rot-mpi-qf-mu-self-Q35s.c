...
#include "imre3d-ms-compgX-rot-mpi-qf-mu-self-Q35s.h"
...
int main(int argc, char **argv) {
   ...
   if (muoutput != NULL) {
      fprintf(muout, "----------------------------------------------------------------------------------------------------------------------------------------------\n");
      fprintf(muout, "Snap      mu             Kin            Pot            Contact        DDI            Dmu(Contact)   Dmu(DDI)       Rot            Lx  \n");
      fprintf(muout, "----------------------------------------------------------------------------------------------------------------------------------------------\n");
      fflush(muout);
   }
   ...
   if (muoutput != NULL) {
      calcmudet(mu, psi, psi_t, dpsi, dpsiL, dpsi_t, psidd2, psidd20, psidd2fft, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj, tmpdpsi1, tmpdpsi2, tmpxmui, tmpymui, tmpzmui, &Lx);
      ...
      fprintf(muout, "%-9d %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", 0, mutot, mu[0], mu[1], mu[2], mu[3], mu[4], mu[5], mu[6], Lx);
      fflush(muout);
   } else {
      calcmu(&mutot, psi, psi_t, dpsi, dpsiL, dpsi_t, psidd2, psidd2fft, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj, tmpdpsi1, tmpdpsi2);
      mutot /= par * norm_psi2;
   }
   ...
   for (snap = 1; snap <= Nsnap; snap ++) {
      for (cnti = 0; cnti < nsteps; cnti ++) {
         ...
         calcluy(psi, cbeta);
         calcluz(psi, cbeta);
         if (optimre == 1) calcnorm(&norm, psi, tmpxi, tmpyi, tmpzi);
      }

      if (optimre == 2) calcnorm(&norm, psi, tmpxi, tmpyi, tmpzi);
      norm_psi2 *= norm * norm;
      norm_psi3 *= norm * norm * norm;
      calcrms(rms, psi, psi_t, tmpxi, tmpyi, tmpzi);
      ...
      if (muoutput != NULL) {
         ...
         fprintf(muout, "%-9li %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", snap, mutot, mu[0], mu[1], mu[2], mu[3], mu[4], mu[5], mu[6], Lx);
         fflush(muout);
      } else {
         calcmu(&mutot, psi, psi_t, dpsi, dpsiL, dpsi_t, psidd2, psidd2fft, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj, tmpdpsi1, tmpdpsi2);
         mutot /= par * norm_psi2;
   ...
   for (cntj = 0; cntj < localNy; cntj ++) {
      for (cnti = 0; cnti < Nx; cnti ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            potdd[cntj][cnti][cntk] = MS * (4. * pi * (3. * kx2[cnti] / (kx2[cnti] + ky2[cntj] + kz2[cntk]) - 1.) / 3.) * tmp;
         }
      }
   }
 ...
 */
void calcmudet(double *mu, double complex ***psi, double complex ***psi_t, double ***dpsi, double ***dpsiL, double ***dpsi_t, double ***psidd2, double ***psidd20, fftw_complex *psidd2fft, double **tmpxi, double **tmpyi, double **tmpzi, double **tmpxj, double **tmpyj, double **tmpzj, double complex **tmpdpsi1, double complex **tmpdpsi2, double ***tmpxmui, double ***tmpymui, double ***tmpzmui, double *Lx) {
   int threadid;
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            for (cntj = 0; cntj < Ny; cntj ++) {
               ...
               dpsiL[cnti][cntj][cntk] = 2. * cimag(tmpdpsi1[threadid][cntj]) * z[cntk] * creal(tmpdpsi2[threadid][cntj]);
            }
         }
      }
      #pragma omp barrier
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            ...
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               dpsiL[cnti][cntj][cntk] += - 2. * cimag(tmpdpsi1[threadid][cntk]) * y[cntj] * creal(tmpdpsi2[threadid][cntk]);
            }
         }
      }
   }
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzmui[6][threadid][cntk] = dpsiL[cnti][cntj][cntk];
            }
   ...
   if (rank == 0) {
      for (cnte = 0; cnte < 7; cnte ++) mu[cnte] = simpint(dx, tmpxmui[cnte][0], Nx);
      *Lx = mu[6];
      mu[6] *= - omega;
   }
...
void calcmu(double *mu, double complex ***psi, double complex ***psi_t, double ***dpsi, double ***dpsiL, double ***dpsi_t, double ***psidd2, fftw_complex *psidd2fft, double **tmpxi, double **tmpyi, double **tmpzi, double **tmpxj, double **tmpyj, double **tmpzj, double complex **tmpdpsi1, double complex **tmpdpsi2) {
   int threadid;
   long cnti, cntj, cntk;
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzi[threadid][cntk] = dpsi2 + (pot[cnti][cntj][cntk] + psi2lin + psidd2lin + h2 * psi3) * psi2 - omega * dpsiL[cnti][cntj][cntk];
            }
