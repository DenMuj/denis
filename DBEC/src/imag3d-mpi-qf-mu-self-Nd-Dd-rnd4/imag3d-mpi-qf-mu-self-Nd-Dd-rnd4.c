...
#include "imag3d-mpi-qf-mu-self-Nd-Dd-rnd4.h"
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("SEED")) == NULL) {
      fprintf(stderr, "SEED is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   seed = atol(cfg_tmp);

   if ((cfg_tmp = cfg_read("DD")) == NULL) {
      fprintf(stderr, "DD is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   Dd = atof(cfg_tmp);
...
void initpsi(double ***psi) {
   long cntd, cnti, cntj, cntk;
   double cpsi;
   double tmp, tmp1, *tmpr;
   MPI_Offset fileoffset;
   MPI_File file = MPI_FILE_NULL;
   double *xd;

   srand48(seed);
   xd = alloc_double_vector(Nd);
   for (cntd = 0; cntd < Nd; cntd ++) xd[cntd] = - 0.5 * (Nd - 1) * Dd + cntd * Dd * (1. + 0.04 * (- 0.5 + drand48()));
   ...
   if (input != NULL) {
      ...
      #pragma omp parallel for private(cntd, cnti, cntj, cntk, tmp, tmp1)
      for (cnti = 0; cnti < localNx; cnti ++) {
         tmp1 = 1.;
         for (cntd = 0; cntd < Nd; cntd ++) tmp1 += 0.02 * exp(-0.5 * (x[offsetNx + cnti] - xd[cntd]) * (x[offsetNx + cnti] - xd[cntd]) / (ux * ux));
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = tmp1 * exp(- 0.5 * (x2[offsetNx + cnti] / (21.5 * 21.5) +  y2[cntj] / (0.9 * 0.9) + z2[cntk] / (2.25 * 2.25)));
