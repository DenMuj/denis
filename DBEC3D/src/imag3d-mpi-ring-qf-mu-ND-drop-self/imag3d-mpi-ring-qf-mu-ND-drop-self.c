...
#include "imag3d-mpi-ring-qf-mu-ND-drop-self.h"
...
*/
   ...
   if (input != NULL) {
      ...
      fprintf(out, "Ring-adapted Gaussian\n               UETA = %.6le, UZ = %.6le\n\n", ueta, uz);
   }
   ...
   if (rmsout != NULL) {
      ...
      if (input != NULL) {
         ...
         fprintf(filerms, "Ring-adapted Gaussian\n               UETA = %.6le, UZ = %.6le\n\n", ueta, uz);
      }
   ...
   if (muoutput != NULL) {
      calcmudet(mu, psi, psi_t, dpsi, dpsi_t, psidd2, psidd20, psidd2fft, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj, tmpxmui, tmpymui, tmpzmui);
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("ND")) == NULL) {
      fprintf(stderr, "ND is not defined in the configuration file\n");
      exit(EXIT_FAILURE);
   }
   ND = atol(cfg_tmp);
...
*/
   ...
   if ((cfg_tmp = cfg_read("UETA")) == NULL) ueta = 0.;
   else ueta = atof(cfg_tmp);
...
void initpsi(double ***psi) {
   long cnti, cntj, cntk, cntd;
   double cpsi;
   double tmp, *tmpr, sigma, invsigma2h, xd, yd, phiini;
   ...
   if (input != NULL) {
      ...
      cpsi = sqrt(pi * sqrt(pi) * ueta * ueta * uz);
      sigma = 2 * pi * Rring / (10 * ND);
      invsigma2h = 0.5 / (sigma * sigma);
      phiini = (0.25 * ND - floor(ND / 4) + 0.8) * 2 * pi / ND;

      #pragma omp parallel for private(cnti, cntj, cntk, cntd, xd, yd)
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               psi[cnti][cntj][cntk] = 0.;
               for (cntd = 0; cntd < ND; cntd++) {
                  xd = Rring * cos(2 * pi * cntd / ND + phiini);
                  yd = Rring * sin(2 * pi * cntd / ND + phiini);
                  psi[cnti][cntj][cntk] += exp(- invsigma2h * ((x[offsetNx + cnti] - xd) * (x[offsetNx + cnti] - xd) +
                                           (y[cntj] - yd) * (y[cntj] - yd) + 0.1 * z2[cntk]));
               }
            }
         }
      }
 ...
 */
void calcmudet(double *mu, double ***psi, double ***psi_t, double ***dpsi, double ***dpsi_t, double ***psidd2, double ***psidd20, fftw_complex *psidd2fft, double **tmpxi, double **tmpyi, double **tmpzi, double **tmpxj, double **tmpyj, double **tmpzj, double ***tmpxmui, double ***tmpymui, double ***tmpzmui) {
   int threadid;
 ...
 *    psidd2fft - array with the squared wave function fft values
 */
void calcnu(double ***psi, double ***psidd2, fftw_complex *psidd2fft) {
   long cnti, cntj, cntk;
