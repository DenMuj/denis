...
#include "imag3d-mpi-qf-mu-self-Nd-Dd-rnd3.h"
...
void initpsi(double ***psi) {
   ...
   if (input != NULL) {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         tmp1 = 0.;
         for (cntd = 0; cntd < Nd; cntd ++) tmp1 += exp(-0.5 * (x[offsetNx + cnti] - xd[cntd]) * (x[offsetNx + cnti] - xd[cntd]) / (ux * ux));
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = (1. + 0.04 * drand48()) * tmp1 * exp(- 0.5 * (y2[cntj] / (uy * uy) + z2[cntk] / (uz * uz)));
