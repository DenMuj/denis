...
#include "real3d-mpi-qf-mu-hm-sprng.h"
...
int main(int argc, char **argv) {
   ...
   omegamt = 0.;
   vgamma2 = 0.5 * par * vgamma * vgamma;
   vnu2 = 0.5 * par * vnu * vnu;
   vlambda2 = 0.5 * par * vlambda * vlambda;
   ...
   calcdonorm(&norm, psi, tmpxi, tmpyi, tmpzi);
   norm_psi2 *= norm * norm;
   ...
   for (snap = 1; snap <= Nsnap; snap ++) {
      for (cnti = 0; cnti < nsteps; cnti ++) {
         omegamt += par * vomegam * dt;
         osc = 1. + eps * sin(omegamt);
         osc *= osc;
         vnu2t = vnu2 * osc;
         vlambda2t = vlambda2 * osc;
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("OMEGAM")) == NULL) {
      fprintf(stderr, "OMEGAM is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   vomegam = atof(cfg_tmp);

   if ((cfg_tmp = cfg_read("EPS")) == NULL) {
      fprintf(stderr, "EPS is not defined in the configuration file.\n");
      exit(EXIT_FAILURE);
   }
   eps = atof(cfg_tmp);
 ...
 *    tmpz - temporary array
 */
void calcdonorm(double *norm, double complex ***psi, double **tmpx, double **tmpy, double **tmpz) {
   int threadid, quick;
   long cnti, cntj, cntk;
   double tmp, alpha;
   void *sendbuf;
   ...
   #pragma omp parallel private(threadid, cnti, cntj, cntk)
   {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            ...
            for (cntk = 0; cntk < Nz; cntk ++) {
               dpsi[cnti][cntj][cntk] += (double) (tmpdpsi2[threadid][cntk] * conj(tmpdpsi2[threadid][cntk]));
            }
         }
      }
   }
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzmui[1][threadid][cntk] = (vgamma2 * x2[offsetNx + cnti] + vnu2t * y2[cntj] + vlambda2t * z2[cntk]) * psi2;
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzi[threadid][cntk] = dpsi2 + ((vgamma2 * x2[offsetNx + cnti] + vnu2t * y2[cntj] + vlambda2t * z2[cntk]) + psi2lin + psidd2lin + h2 * psi3) * psi2;
            }
   ...
   for (cnti = 0; cnti < localNx; cnti ++) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ...
            psi2lin = g * psi2;
            psidd2lin = gd * psidd2[cnti][cntj][cntk];
            tmp = dt * ((vgamma2 * x2[offsetNx + cnti] + vnu2t * y2[cntj] + vlambda2t * z2[cntk]) + psi2lin + psidd2lin + h2 * psi3);
