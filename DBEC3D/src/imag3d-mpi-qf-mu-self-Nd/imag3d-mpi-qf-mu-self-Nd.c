...
#include "imag3d-mpi-qf-mu-self-Nd.h"
...
void initpsi(double ***psi) {
   ...
   for (cntd = 0; cntd < Nd; cntd ++) xd[cntd] = - 0.5 * (Nd - 1) * 5. + cntd * 5.;
   ...
   if (input != NULL) {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         tmp1 = 0.;
         for (cntd = 0; cntd < Nd; cntd ++) tmp1 += 0.05 * exp(-0.5 * (x[offsetNx + cnti] - xd[cntd]) * (x[offsetNx + cnti] - xd[cntd]) / (ux * ux));
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
