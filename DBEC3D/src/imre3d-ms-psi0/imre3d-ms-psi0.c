...
int main(int argc, char **argv) {
   ...
   double norm, mu[8], mutot, mutotold, Lx, den0;
   double *rms;
   double complex **cbeta, psi0;
...
//   q3 = 1. + 0.3 * edd * edd;
   ...
   if ((fabs(edd) >= 1e-10) && (fabs(edd - 1.) >= 1e-10)) {
      LLL = clog(fabs(-1. + edd)/(1. + 5.*edd + 2.*csqrt(3.)*csqrt(edd)*csqrt(1. + 2.*edd)));

      CC[1] = creal((-30.*csqrt(edd)*csqrt(1. + 2.*edd) - 500.*cpow(edd,1.5)*csqrt(1. + 2.*edd) + 74.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
              csqrt(3.)*cpow(-1. + edd,2)*(-5. + 43.*edd)*LLL)/(1152.*cpow(edd,1.5)*csqrt(pi)));
      CC[2] = CC[1];

      CC[3] = creal((30.*csqrt(edd)*csqrt(1. + 2.*edd) - 160.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 98.*cpow(edd,2.5)*csqrt(1. + 2.*edd) -
              csqrt(3.)*(-1. + edd)*(5. + edd*(54. + 31.*edd))*LLL)/(576.*cpow(edd,1.5)*csqrt(pi)));

      CC[4] = creal((-5.*(6.*csqrt(edd)*csqrt(1. + 2.*edd) + 4.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 34.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
              csqrt(3.)*cpow(-1. + edd,2)*(1. + edd)*LLL))/(1152.*cpow(edd,1.5)*csqrt(pi)));
      CC[5] = CC[4];

      CC[6] = creal((5.*(6.*csqrt(edd)*csqrt(1. + 2.*edd) + 16.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 10.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
              csqrt(3.)*(1. + edd*(5. + (5 - 11.*edd)*edd))*LLL))/(576.*cpow(edd,1.5)*csqrt(pi)));

      CC[7] = creal((5.*(6.*csqrt(edd)*csqrt(1. + 2.*edd) + 4.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 34.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
              csqrt(3.)*cpow(-1. + edd,2)*(1. + edd)*LLL))/(192.*cpow(edd,1.5)*csqrt(pi)));
      CC[8] = CC[7];

      CC[9] = creal((5.*(-6.*csqrt(edd)*csqrt(1. + 2.*edd) - 16.*cpow(edd,1.5)*csqrt(1. + 2.*edd) + 10.*cpow(edd,2.5)*csqrt(1. + 2.*edd) -
              csqrt(3.)*(1. + edd*(5. + (5 - 11.*edd)*edd))*LLL))/(96.*cpow(edd,1.5)*csqrt(pi)));

      CC[10] = creal((30.*csqrt(edd)*csqrt(1. + 2.*edd) - 40.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 182.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
               csqrt(3.)*cpow(-1. + edd,2)*(5. + 11.*edd)*LLL)/(288.*cpow(edd,1.5)*csqrt(pi)));
      CC[11] = CC[10];

      CC[12] = creal((-30.*csqrt(edd)*csqrt(1. + 2.*edd) - 110.*cpow(edd,1.5)*csqrt(1. + 2.*edd) + 44.*cpow(edd,2.5)*csqrt(1. + 2.*edd) +
               csqrt(3.)*(-1. + edd)*(5. + edd*(27. + 58.*edd))*LLL)/(144.*cpow(edd,1.5)*csqrt(pi)));

      CC[13] = creal((30.*csqrt(edd)*csqrt(1. + 2.*edd) + 200.*cpow(edd,1.5)*csqrt(1. + 2.*edd) - 134.*cpow(edd,2.5)*csqrt(1. + 2.*edd) -
               csqrt(3.)*cpow(-1. + edd,2)*(-5. + 13.*edd)*LLL)/(288.*cpow(edd,1.5)*csqrt(pi)));
      CC[14] = CC[13];

      CC[15] = creal((-30.*csqrt(edd)*csqrt(1. + 2.*edd) + 10.*cpow(edd,1.5)*csqrt(1. + 2.*edd) + 68.*cpow(edd,2.5)*csqrt(1. + 2.*edd) -
               csqrt(3.)*(5. + edd*(34. + (7 - 46.*edd)*edd))*LLL)/(144.*cpow(edd,1.5)*csqrt(pi)));
   }
   ...
   for (cnte = 0; cnte < 16; cnte ++) CC[cnte] *= GRAD * pow(as * BOHR_RADIUS / aho, 1.5) * sqrt(Na);
   ...
   if (rank == 0) {
      if (output != NULL) {
         sprintf(filename, "ex-%s.txt", output);
         out = fopen(filename, "w");
      } else out = stdout;
   } else out = fopen("/dev/null", "w");

   muout = fopen("/dev/null", "w");

   filerms = fopen("/dev/null", "w");

   sprintf(final, "%s.bin", finalpsi);
   ...
   if (cfg_read("G") != NULL) {
      ...
      fprintf(out, "Contact: Natoms = %.6le, as = %.6le * a0, G = %.6le, G * par = %.6le\n", (double) Na, as, g / par, g);
   }
   ...
   fprintf(out, "ADD_ONE_VORTEX = %li, ADD_RANDOM_PHASE = %li", ADD_ONE_VORTEX, ADD_RANDOM_PHASE);                      
   if (ADD_RANDOM_PHASE == 1) fprintf(out, ", SEED = %li", seed);
   fprintf(out, "\n\n");

   if (cfg_read("TAU") != NULL) fprintf(out, "       Unit of time: tau = %.6le s\n", tau);
   fprintf(out, "MUREL = %.6le\n\n", murel);

//   fprintf(out, "-------------------------------------------------------------------------------------------------------------------------------\n");
   fprintf(out, "edd            N              as             Norm0          Norm           mu             <r>            <x>            <y>            <z>            Den0           q3             q5\n");
//   fprintf(out, "-------------------------------------------------------------------------------------------------------------------------------\n");
   fflush(out);
   ...
   if (rmsout != NULL) {
      ...
      if (cfg_read("G") != NULL) {
         ...
         fprintf(filerms, "Contact: Natoms = %.6le, as = %.6le * a0, G = %.6le, G * par = %.6le\n", (double) Na, as, g / par, g);
      }
      ...
      fprintf(filerms, "ADD_ONE_VORTEX = %li, ADD_RANDOM_PHASE = %li", ADD_ONE_VORTEX, ADD_RANDOM_PHASE);
      if (ADD_RANDOM_PHASE == 1) fprintf(filerms, ", SEED = %li", seed);
      fprintf(filerms, "\n\n");

      if (cfg_read("TAU") != NULL) fprintf(filerms, "       Unit of time: tau = %.6le s\n", tau);
      fprintf(filerms, "MUREL = %.6le\n\n", murel);  
   ...
   if (muoutput != NULL) {
      ...
      fprintf(muout, "%-9d %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", 0, mutot, mu[0], mu[1], mu[2], mu[3], mu[4], mu[5], mu[6], mu[7], Lx);
      fflush(muout);
   } else {
   ...
   psi0 = psi[Nx2][Ny2][Nz2];
   den0 = fabs(psi0);
   den0 *= den0;

   fprintf(out, "%-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le %-14.6le\n", edd, (double) Na, as, norm_psi2, norm_psi2 + h4 * norm_psi3, mutot, rms[0] / sqrt(norm_psi2), rms[1] / sqrt(norm_psi2), rms[2] / sqrt(norm_psi2), rms[3] / sqrt(norm_psi2), den0, q3, q5);
   fflush(out);
...
void readpar(void) {
   ...
   if ((cfg_tmp = cfg_read("ADD_ONE_VORTEX")) != NULL) ADD_ONE_VORTEX = atol(cfg_tmp);
...
void initpsi(double complex ***psi) {
   ...
   if (input != NULL) {
      MPI_File_open(MPI_COMM_WORLD, final, MPI_MODE_RDONLY, MPI_INFO_NULL, &file);
      ...
      cpsi = sqrt(pi * sqrt(pi) * ux * uy * uz / 8.);
   ...
   if ((ADD_RANDOM_PHASE == 1) || (ADD_ONE_VORTEX == 1)) {
      for (cntj = 0; cntj < Ny; cntj ++) {
         for (cntk = 0; cntk < Nz; cntk ++) {
            ctmpyz[cntj][cntk] = 1.;
            if(ADD_RANDOM_PHASE == 1) ctmpyz[cntj][cntk] *= cexp(2 * pi * I * drand48());
            if(ADD_ONE_VORTEX == 1) ctmpyz[cntj][cntk] *= (y[cntj] + I * z[cntk]);
         }
      }
