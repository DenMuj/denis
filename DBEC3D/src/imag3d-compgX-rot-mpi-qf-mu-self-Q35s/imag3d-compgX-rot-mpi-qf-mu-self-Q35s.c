...
#include "imag3d-compgX-rot-mpi-qf-mu-self-Q35s.h"
...
int main(int argc, char **argv) {
   ...
   if (muoutput != NULL) {
      fprintf(muout, "-------------------------------------------------------------------------------------------------------------------------------\n");
      fprintf(muout, "Snap      mu             Kin            Pot            Contact        DDI            Dmu(Contact)   Dmu(DDI)       Rot \n");
      fprintf(muout, "-------------------------------------------------------------------------------------------------------------------------------\n");
      fflush(muout);
   }
   ...
   if (muoutput != NULL) {
      calcmudet(mu, psi, psi_t, dpsi, dpsiL, dpsi_t, psidd2, psidd20, psidd2fft, tmpxi, tmpyi, tmpzi, tmpxj, tmpyj, tmpzj, tmpdpsi1, tmpdpsi2, tmpxmui, tmpymui, tmpzmui);
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               ...
               tmpzmui[6][threadid][cntk] = - omega * dpsiL[cnti][cntj][cntk];
            }
