...
#include "imag3d-mpi-qf-mu-self-Nd-Dd-rnd2.h"
...
void initpsi(double ***psi) {
   ...
   for (cntd = 0; cntd < Nd; cntd ++) xd[cntd] = - 0.5 * (Nd - 1) * Dd + cntd * Dd * (1. + 0.02 * (- 0.5 + drand48()));
   ...
   if (input != NULL) {
      ...
      for (cnti = 0; cnti < localNx; cnti ++) {
         ...
         for (cntj = 0; cntj < Ny; cntj ++) {
            for (cntk = 0; cntk < Nz; cntk ++) {
               tmp = (1. + 0.01 * drand48()) * tmp1 * exp(- 0.5 * (y2[cntj] / (uy * uy) + z2[cntk] / (uz * uz)));
